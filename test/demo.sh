...
# get canister IDs
export DEX_PRINCIPLE=$(dfx canister id twype_token)
...
export ICP_DEPOSIT_ADDR_USER1=$(dfx canister call twype_token getDepositAddress | tr -d '\n' | sed 's/,)/)/')
...
export ICP_DEPOSIT_ADDR_USER2=$(dfx canister call twype_token getDepositAddress | tr -d '\n' | sed 's/,)/)/')
...
dfx identity use user1
dfx canister call twype_token placeOrder '(principal '\"$LEDGER_ID\"', 3, principal '\"$GOLDEN_ID\"', 200)'
dfx identity use user2
dfx canister call twype_token placeOrder '(principal '\"$GOLDEN_ID\"', 200, principal '\"$LEDGER_ID\"', 3)'
dfx canister call twype_token getBalances
dfx canister call twype_token getBalances | grep 999_800
dfx identity use user1
dfx canister call twype_token getBalances
dfx canister call twype_token getBalances | grep 39_997
dfx identity use default
echo "PASS"
