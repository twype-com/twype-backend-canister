<script>
  ...
  onMount(async () => {
    ...
    if ($auth.loggedIn) {
      ...
      const rt1SellPrice = await backendActor.getRTSellPrice(1, 100);
  ...
  async function SellT() {
    const result = await backendActor.sellRoomToken(1, 100);
    if (result.Ok) {
    ...
    <div class="balance-container">
      ...
      <table class="balance-table">
        <thead>
          ...
          <th>RT1 Balance</th>
          <th>1k RT1 Buy Price</th>
          <th>1k RT1 Sell Price</th>
          <th>RT1 Supply</th>
        </thead>
        <tbody>
          {#each $userBalances as balance}
            <tr>
              ...
              <td>
                {balance && balance.canisterBalance
                  ? balance.canisterBalance.toLocaleString()
                  : "0"}
              </td>
              <td>
                <div>
                  <button
                    title="deposit"
                    disabled={balance.canisterBalance <= 0 ||
                      balance.canisterBalance < depositAmount}
                    on:click={() => depositT(balance.principal)}
                  >
                  ...
                  <button
                    title="Withdraw"
                    disabled={balance.dexBalance <= 0}
                    on:click={() => beginWithdrawProcess(balance.principal)}
                  >
                    <div class="add-btn-text">
                      {#if withdrawingAmount && currentToken === balance.principal}
                        <div class="loader"></div>
                      {:else if withdrawing && currentToken === balance.principal}
                        <div>
                          <input
                        ...
                        <button
                          disabled={withdrawAmount <= 0 ||
                            withdrawAddress === ""}
                          on:click={() => withdrawT(balance.principal)}
                          ><FontAwesomeIcon icon="check" /></button
                        >
                        <button on:click={(e) => cancelWithdrawProcess(e)}
                  ...
                  <button title="sell" on:click={() => SellT()}>
                    <div class="add-btn-text">sell</div>
                  </button>
                </div>
              </td>
              <td>
              ...
              <td>
                {balance && balance.rt1SellPrice
                  ? balance.rt1SellPrice.toLocaleString()
                  : "0"}
              </td>
              <td>
    ...
    <div class="deposit-address">
      ...
      <span class="deposit-value-text">
        ...
        {:else}
          ...
          <span
            class="copy-icon"
            on:click={() => copyDepositAddress(toHexString(depositAddressBlob))}
          >
