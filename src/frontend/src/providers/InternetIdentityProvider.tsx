...
export const InternetIdentityProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const initializeInternetIdentity = useCallback(async () => {
    ...
    const twypeTokenActor = createCanisterActor(
      agent,
      twypeTokenIdlFactory,
      import.meta.env.VITE_TWYPE_TOKEN_CANISTER_ID,
    )
    ...
    const ledgerActor = createCanisterActor(
      agent,
      ledgerIdlFactory,
  ...
  const withdraw = useCallback(
    async (amount: number) => {
      ...
      const response = await twypeTokenActor.withdraw(balance.canisterPrincipal, amount, principal)
